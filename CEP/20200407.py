...
def genAccount():
    ...
    while not done:
        ...
        if username=="":
            ...
            agree="Your name is "+username+", is that correct? (y/n)\n$ "
            if agree.lower()=="y":
                done=True
                break
            elif agree.lower()=="n":
                ...
                while agree.lower() not in ["y","n"]:
                    agree=input("Your name is "+username+", is that correct? (y/n)\n$ ")
                if agree.lower()=="y":
                    done=True
                    break
                elif agree.lower()=="n":
    ...
    # Welcome Message
    welcomeMsg = "\nWelcome, "+username+"!\n"+"""You are a hacker.
Now, you are back on another mission.
...
def cd(args):
    if len(args)<1:
        ...
        print(current)
        if current in ROOTFOLDERS:
            if elevated:
                current_dir.append(args[0])
            elif not elevated:
                print("You must be elevated to do this")
        else:
            current_dir.append(args[0])
