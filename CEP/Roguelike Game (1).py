...
enemies =("B","T","G")
inventory = [["Keys > ",0 ],["Rubies > ",0 ], ["Health Pots > ", 20], ["Weapon: ","Wood Sword"],["Armour: ", "Rugged Clothes"]]
sceneChange = False
gameOver = [False, "Nil"]
...
def shop():
    ...
    while True:
        ...
        if cmd == "help":
            ...
            print_slow("\nAvaliable Items:")
            print("\n1 - 6 Health Pots [15 Rubies]", "2 - Iron Sabre (25 Attack)[30 Rubies]", "3 - Polished Armor (10 Defence)[30 Rubies]", "4 - Void Blade (33 Attack)[43 Rubies]", "5 - Shade Cloak (20 Defence)[50 Rubies]", "6 - Crystal Heart (+20 to Max Health)[20 Rubies]", sep = "\n")
            ...
            if itemNumber == 1:
                ...
                if avaliableRubies >= cost:
                    inventory[1][1] -= cost
                    inventory[2][1] += 6
                    print_slow ("\n6 Health Pots were added to your inventory!")
...
def instructions():
    ...
    time.sleep(0.25)
    print_slow("Enemy and block index:\n")
    print("A > Adventurer", "O > Old Man", "- > Blank Space", "| or ^ > Door", "# > Wall", "S > Shopkeeper", "C > Coin", "B > Bat", "T > Troll", "G > Goblin", "C > Treasure Chest", sep = "\n", end = "\n")
    time.sleep(5.0)
    line()
...
def load_tile(scene):
    num_columns = len(scene)//9
    for columns in range(num_columns):
        printableRow = ""
        for rows in range(9):
            printableRow += (scene[rows + (columns*9)]) + (5 * " ")
        print(printableRow,"\n")
...
def fight(enemy):
    ...
    while enemy_hp > 0:
        ...
        if cmd == "help":
            ...
            print_slow("\nAvaliable Commands:\n")
            print("attack > Attack the enemy", "heal > Consume a health pot and heal 30HP", "guard > Will negate or reduce damage taken, dealing some damage to the enemy", "hp > Displays player HP", "enem_hp > Displays the enemy's hp", sep = "\n")
            time.sleep(0.5)
            line()
        ...
        #Guard Command
            ...
            print_slow("\nYou held up your trusty spiky shield and attempted to block the attack")
...
def command(scene):
    ...
    while True:
        ...
        if cmd == "help":
            print_slow("\nCommands:\n")
            print("help > Display commands","inventory > View your inventory", "quit > Quit the game", "hp > Display player HP", "w > up","a > left","s > down","d > right","dam > Displays weapon damage", "def > Displays armor defense", "admin > Activates the admin cheat. Infinite items, damage, and defence. USE ONLY WHEN NEEDED\n",sep = "\n")
...
#To be called whenever a new game is started so as to reset all global variables
def reset():
