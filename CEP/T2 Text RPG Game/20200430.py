...
def dupList(list):
    # duplicate a list manually becuase when a=b, the changes of a and b are "synced"
    result=[]
    for i in list:
        result.append(i)
    return result
...
def cd(args):
    ...
    if len(args)<1:
        ...
        if len(args)>1:
            ...
            current_dir=[]
            for i in args:
                print(current_dir,current_dictionary())
...
while True:
    try:
        ...
        if len(current_dir)>=2:
            ...
            raw = input(username+"@"+"ThinkPadX1C:"+"/"+"/".join(current_dir)+" $ ")
        print("Afasffafa")
        print("b4",current_dir)
        inputRun(raw)
        print("aft",current_dir)
    except RuntimeError:
        break
